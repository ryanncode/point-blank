...
`LIST FROM [FILES|BLOCKS] [IN] ["path"|this.file|this.folder|workspace] [EXT "extension"] WHERE <conditions> SORT BY <key> [ASC|DESC]`
...
### `IN "path" | this.file | this.folder | workspace`

This optional clause defines the scope of your search. (Note: the `IN` keyword is optional, so `FROM FILES "notes"` works too).
*   `"relative/path"`: Searches within a specific folder or file.
...
*   `workspace`: Searches across your entire VS Code workspace (this is the default if no scope is specified).

### `EXT "extension"`

This optional clause allows you to include files other than markdown files (which are the default). 
*   `EXT ".pdf"`: Searches for `.pdf` files.
*   `EXT "*"`: Searches for all file types.
...
*   **Operators:**
    ...
    *   `::`: Legacy existence check or equality.
...
**Intelligent Numerical Sorting & File Properties**
Point Blank will automatically perform numerical sorting if the properties represent valid numbers. Furthermore, three "virtual" properties are injected during queries that you can sort by:
*   `filename`: The name of the file.
*   `creation date`: A numerical timestamp of file creation.
*   `last updated`: A numerical timestamp of the file's last modification.

Example: `SORT BY last updated DESC`
...
3.  **List all PDFs in the assets folder:**
    ```
    LIST FROM FILES "assets" EXT ".pdf"
    ```

### Scoped Queries

4.  **Find all "Idea" blocks within the current file:**
    ```
    LIST FROM BLOCKS IN this.file WHERE Type == "Idea"
    ```

5.  **List all "Project" files within the current folder (and its subfolders):**
    ```
...
6.  **List all "Task" blocks with "High" priority:**
    ```
    LIST FROM BLOCKS WHERE Type == "Task" AND Priority == "High"
    ```

7.  **List all "Task" blocks that are not "Done":**
    ```
    LIST FROM BLOCKS WHERE Type == "Task" AND Status != "Done"
    ```

8.  **Find all "Meeting" blocks that occurred after January 1, 2024:**
    ```
...
10. **List all "Project" files, sorted by `last updated` date in descending order:**
    ```
    LIST FROM FILES WHERE Type == "Project" SORT BY last updated DESC
    ```
...
1.  **Write the Query:** Type your query inside a comment block using the `pb:query` prefix:
    ```markdown
    <!-- pb:query LIST FROM BLOCKS WHERE Type == "Task" AND Status == "Pending" -->
    ```
    *(Note: The older `pointblank:query` prefix is also supported for backward compatibility).*
...
*   **`Point Blank: Insert Query by Type`** (`pointblank.insertTypeQuery`)
    ...
    *   Example Query Inserted: `<!-- pb:query LIST FROM BLOCKS WHERE Type == Task -->`
